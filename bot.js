...
client.on("message", msg => {
  ...
  if (msg.content.toLowerCase().startsWith(prefix + "kick ")) {
    ...
    mem.kick().then(() => {
      msg.channel.send(mem.displayName + " has successfully been kicked from Rahul World by " + msg.author.username + "!");
    }).catch(e => {
  ...
  if (msg.content.toLowerCase().startsWith(prefix + "ban ")) {
    ...
    mem.ban(mc).then(() => {
      msg.channel.send(mem.displayName + " has successfully been banned from Rahul World by " + msg.author.username + " for " + mc + " days!");
    }).catch(e => {
      msg.channel.send("An error occured!");
    });
  }
...
client.login("NTczMTMyNzczOTM0NzU5OTQ4.XMmZyw.K_IlnDCiQbUhV3XrozUh3SvEnAA");
